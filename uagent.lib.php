...
/*
 * Auto Create List User Agent Random
 */
...
function osx_version() {
    return "10_" . rand(4, 9) . '_' . rand(0, 9);
}

function chrome_version() {
    return rand(10, 42) . '.0.' . rand(800, 899) . '.0';
}
...
function firefox($arch) {
    $ver = array_random(array(
        'Gecko/' . date('Ymd', rand(strtotime('2011-1-1'), time())) . ' Firefox/' . rand(5, 40) . '.0',
        'Gecko/' . date('Ymd', rand(strtotime('2011-1-1'), time())) . ' Firefox/' . rand(5, 40) . '.0.1',
...
function safari($arch) {
    ...
    switch( $arch ) {
        ...
        case 'iphone':
            return '(iPod; U; CPU iPhone OS ' . rand(3, 8) . '_' . rand(0, 3) . " like Mac OS X; {lang}) AppleWebKit/$saf (KHTML, like Gecko) Version/" . rand(3, 4) . ".0.5 Mobile/8B" . rand(111, 119) . " Safari/6$saf";
        case 'win':
        default:
            $nt = nt_version();
...
function random_uagent() {
    ...
    $user_agent = str_replace('{proc}', array_random($proc[$os]), $ua);
    return str_replace('{lang}', array_random($lang), $user_agent);
}
